...
void Sparkline(const char* id, const double* values, int count, double min_v, double max_v, const ImVec2& size, int offset) {
...
void RenderTelemetryRow(const char* label, double value, const char* unit, const std::vector<double>& history_data, double min_val, double max_val) {
    ImGui::TableNextRow();
    ...
    if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("COLUMN_NAME")) {
            ...
            if (std::getline(ss, archiveName, ':') && std::getline(ss, columnName, ':')) {
                std::string metricIdentifier = archiveName + ":" + columnName;
                for (const auto& metric : customMetrics) {
                    if (metric.unique_id == metricIdentifier) return;
                }
...
void Window::Statistics::renderTable() {
    ...
    static ImGuiTableFlags flags = ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable;
    ...
    if (ImGui::BeginTable("##telemetry_table", column_count, flags)) {
        ...
        if (showGraphs) { ImGui::TableSetupColumn("Histórico", ImGuiTableColumnFlags_WidthStretch); }
        ...
        if (!telemetryHistory.empty()) {
            ...
            auto extract_history = [&](auto extractor) {
                ...
                for (const auto& data : telemetryHistory) { history_values.push_back(extractor(data)); }
            };
            ...
            for (int i = 0; i < customMetrics.size(); ++i) {
                ...
                if (showGraphs) {
                    ...
                    if (!metric.values.empty()) {
                        ...
                        if (new_range_v <= 0) new_range_v = 1.0;
                        ...
                        Sparkline("##spark_custom", metric.values.data(), window_size, display_min, display_max, ImVec2(-1, 35), static_cast<int>(metric.current_offset));
                    }
                    ImGui::PopID();
                }
                ...
                ImGui::PushID(i);
                if (ImGui::Button("...")) { ImGui::OpenPopup(popup_id.c_str()); }
                ...
                ImGui::SameLine();
                if (ImGui::Button("X")) { metric_to_remove = i; }
                ImGui::PopID();
            }
        }
        ImGui::EndTable();
