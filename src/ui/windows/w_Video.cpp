...
void Window::Video::pause() { m_isPlaying = false; }

bool Window::Video::isPlaying() const { return m_isPlaying; }

bool Window::Video::isLoaded() const { return m_formatCtx != nullptr; }

double Window::Video::getCurrentTime() const { return m_currentTime; }

double Window::Video::getDuration() const { return m_duration; }
        ...
        if (m_packet->stream_index == m_videoStreamIndex) {
            if (avcodec_send_packet(m_codecCtx, m_packet) == 0) {
                ...
                if (ret == 0) {
                    ...
                    sws_scale(m_swsCtx, (const uint8_t* const*)m_frame->data, m_frame->linesize, 0, m_videoHeight,
                              pixels, pitch);
...
bool Window::Video::loadVideo(const std::string& filepath) {
    ...
    m_swsCtx = sws_getContext(m_videoWidth, m_videoHeight, m_codecCtx->pix_fmt, m_videoWidth, m_videoHeight,
    ...
    m_texture =
        SDL_CreateTexture(m_renderer, SDL_PIXELFORMAT_RGBA32, SDL_TEXTUREACCESS_STREAMING, m_videoWidth, m_videoHeight);
...
void Window::Video::render() {
    ...
    if (ImGui::Button("Carregar Vídeo")) {
        ...
        const char* filepath =
            tinyfd_openFileDialog("Selecione um vídeo", "", 2, filterPatterns, "Arquivos de Vídeo", 0);
        if (filepath) {
            loadVideo(filepath);
        }
    }
