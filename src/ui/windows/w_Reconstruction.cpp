    ...
    if (window_is_expanded) {
        if (m_loaded) {
            ...
            */
                    ...
                    if (latDataPtr && lonDataPtr && !latDataPtr->empty() && !lonDataPtr->empty()) {
                        ...
                        if (m_rotateMap && endIdx > 0) {
                            ...
                            if (dx * dx + dy * dy > 1e-10) {
                                ...
                                // Normalize
                                while (m_currentHeading < -pi)
                                    m_currentHeading += 2.0 * pi;
                                while (m_currentHeading > pi)
                                    m_currentHeading -= 2.0 * pi;
                            }
                        }
            ...
            // A quick fix is to multiply the required tiles by 1.5 when rotating to ensure the corners don't get black
            // triangles.
                ...
                try {
                    ...
                    if (latDataPtr && lonDataPtr) {
                        ...
                        if (!latData.empty() && !lonData.empty()) {
                            ...
                            size_t startIdx  = (m_limitPoints && numPoints > (size_t)m_numPointsToShow)
                                                   ? numPoints - m_numPointsToShow
                                                   : 0;
