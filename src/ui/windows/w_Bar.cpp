...
Window::Bar::Bar(const std::string& title) : IWindow() {
    ...
    m_confLL.color[0]     = 0.8f;
    m_confLL.color[1]     = 0.1f;
    m_confLL.color[2]     = 0.1f;
    m_confLL.color[3]     = 1.0f; // Red
    m_confL.color[0]      = 0.8f;
    m_confL.color[1]      = 0.5f;
    m_confL.color[2]      = 0.0f;
    m_confL.color[3]      = 1.0f; // Orange
    m_confNormal.color[0] = 0.0f;
    m_confNormal.color[1] = 0.8f;
    m_confNormal.color[2] = 0.0f;
    m_confNormal.color[3] = 1.0f; // Green
    m_confH.color[0]      = 0.8f;
    m_confH.color[1]      = 0.5f;
    m_confH.color[2]      = 0.0f;
    m_confH.color[3]      = 1.0f; // Orange
    m_confHH.color[0]     = 0.8f;
    m_confHH.color[1]     = 0.1f;
    m_confHH.color[2]     = 0.1f;
    m_confHH.color[3]     = 1.0f; // Red
...
void Window::Bar::render() {
        ...
        if (!dataPtr->empty()) {
            ...
            if (!m_useManualLimits) {
                ...
                if (m_useFormula) {
                    ...
                    double v2 = dataMax * m_multiplier + m_offset;
                    dataMin   = std::min(v1, v2);
                    dataMax   = std::max(v1, v2);
                }
                ...
                if (m_colorBarMode == 2) {
                    if (m_confLL.enabled)
                        minL = std::min(minL, m_threshLL);
                    if (m_confL.enabled)
                        minL = std::min(minL, m_threshL);
                    if (m_confH.enabled)
                        maxL = std::max(maxL, m_threshH);
                    if (m_confHH.enabled)
                        maxL = std::max(maxL, m_threshHH);
                }
            }
        } else {
            val = 0.0;
    ...
    if (hasVal) {
        if (m_colorBarMode == 1 || m_colorBarMode == 3) {
            ...
            if (m_colorBarMode == 1) {
                ...
                interpolatedColor[0] = col.x;
                interpolatedColor[1] = col.y;
                interpolatedColor[2] = col.z;
                interpolatedColor[3] = col.w;
            } else {
    ...
    if (ImGui::BeginPopupContextWindow()) {
        ...
        if (ImGui::BeginMenu("Customização de Cores")) {
            ...
            if (ImGui::RadioButton("Gradiente", m_colorBarMode == 1)) {
                m_colorBarMode = 1;
            }
            if (ImGui::RadioButton("Gradiente Manual", m_colorBarMode == 3)) {
                m_colorBarMode = 3;
            }
            ...
            if (m_colorBarMode == 1 || m_colorBarMode == 3) {
                ...
                ImGui::PushItemWidth(120.0f);
                ImGui::InputDouble("Min Val##grad", &m_gradMinVal);
                ImGui::InputDouble("Max Val##grad", &m_gradMaxVal);
                ImGui::PopItemWidth();
                ...
                if (m_colorBarMode == 1) {
                    ...
                    if (ImPlot::ShowColormapSelector("##colormap_bar")) {
                    ...
                    ImGui::ColorEdit4("Cor Min##bar", m_gradMinColor,
                                      ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
                    ImGui::ColorEdit4("Cor Max##bar", m_gradMaxColor,
                                      ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
                    ImGui::Checkbox("Inverter Cores", &m_reverseColormap);
                }
            } else if (m_colorBarMode == 2) {
                ImGui::Separator();
                auto renderBarThreshConf = [](const char* label, double* thresh, BarThresholdConfig& conf,
                                              bool hasThresh = true) {
                    ImGui::PushID(label);
                    ...
                    if (conf.enabled) {
                        ...
                        ImGui::ColorEdit4("Cor##col", conf.color,
                                          ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar);
                    } else {
    ...
    if (!m_hasData || !dataPtr || dataPtr->empty()) {
        ...
        // Colors
        ImU32 barColU32 =
            ImGui::ColorConvertFloat4ToU32(ImVec4(targetColor[0], targetColor[1], targetColor[2], targetColor[3]));
        ...
        if (m_orientation == 0) {
            ...
            float barHeight = avail.y - headerHeight - footerHeight - (m_showColumnName ? spacing : margin) -
                              (footerHeight > 0.0f ? spacing : margin);
            ...
            if (m_colorBarMode == 2) {
                auto drawStripe = [&](double threshVal, const float* col) {
                    ...
                    if (ratio >= 0.0 && ratio <= 1.0) {
                        ...
                        ImGui::GetWindowDrawList()->AddLine(ImVec2(barMin.x, stripeY), ImVec2(barMax.x, stripeY),
                                                            IM_COL32(0, 0, 0, 255), thickness + 1.5f);
                ...
                if (m_confLL.enabled)
                    drawStripe(m_threshLL, m_confLL.color);
                if (m_confL.enabled)
                    drawStripe(m_threshL, m_confL.color);
                if (m_confH.enabled)
                    drawStripe(m_threshH, m_confH.color);
                if (m_confHH.enabled)
                    drawStripe(m_threshHH, m_confHH.color);
            }
            ...
            if (m_colorBarMode == 2) {
                auto drawStripe = [&](double threshVal, const float* col) {
                    ...
                    if (ratio >= 0.0 && ratio <= 1.0) {
                        ...
                        // Draw black background line for high contrast against any fill/background
                        ImGui::GetWindowDrawList()->AddLine(ImVec2(stripeX, barMin.y), ImVec2(stripeX, barMax.y),
                                                            IM_COL32(0, 0, 0, 255), thickness + 1.5f);
