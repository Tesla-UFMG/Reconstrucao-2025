...
try:
    with open(TTY_PATH, "w") as f:
        ...
        loop_count = 0  # Contador para gerenciar frequências diferentes
        ...
        while True:
            ...
            # ---------------------------------------------------------
            # PACOTES DE 10Hz (Atualizam em todos os ciclos do loop)
            # ---------------------------------------------------------
            ...
            # ---------------------------------------------------------
            # PACOTE DE 5Hz (Atualiza apenas a cada 2 ciclos)
            # ---------------------------------------------------------
            if loop_count % 2 == 0:
                # Calcula a velocidade angular (radianos por segundo)
                omega = (2 * math.pi) / TEMPO_DE_VOLTA
                
                # Equação paramétrica da elipse (Carro contornando a pista)
                # O seno e cosseno defasados criam o movimento circular
                lat_sim = LAT_CENTER + LAT_AMP * math.sin(omega * t)
                lon_sim = LON_CENTER + LON_AMP * math.cos(omega * t)
                
                # Adiciona ao buffer com precisão de 8 casas decimais
                linhas.append(f"36,{lat_sim:.8f},{lon_sim:.8f},0,0.0,0,0,0,0")
            
            # Escreve todos os IDs processados neste ciclo de uma vez
            for linha in linhas:
                f.write(linha + "\n")
                print(linha)
